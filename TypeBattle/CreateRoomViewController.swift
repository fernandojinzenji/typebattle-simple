...
class CreateRoomViewController: UIViewController, UITextFieldDelegate, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.categorySegmentedControl.setTitleTextAttributes([NSAttributedString.Key(rawValue: convertFromNSAttributedStringKey(NSAttributedString.Key.font)): font], for: .normal)
    ...
    @IBAction func createRoom(_ sender: UIButton) {
        ...
        self.savedGameSession = manager.createGameSession(lobby: lobby, someRandomText: NetworkManager.getHardcodedText(category: category), persistInFirebase: true)
    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        ...
        case "goto-lobby-segue":
            // check if room name have at least 3 characters
            if ((roomNameTextField.text?.count)! <= 3) {
                return false
            }
            else {
                return true
